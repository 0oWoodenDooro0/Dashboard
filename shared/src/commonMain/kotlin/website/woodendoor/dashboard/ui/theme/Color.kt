package website.woodendoor.dashboard.ui.theme

import androidx.compose.ui.graphics.Color

// Background & Surface
val DarkBackground = Color(0xFF111216)
val DarkSurface = Color(0xFF1A1B20)
val DarkSurfaceElevated = Color(0xFF22242B)
val DarkSurfaceHighlight = Color(0xFF2B2E38)

// Borders & Dividers
val DarkBorder = Color(0xFF2D3039)
val DarkBorderHover = Color(0xFF454957)
val DarkDivider = Color(0xFF252730)

// Status Colors
val StatusHealthy = Color(0xFF10B981) // Emerald 500
val StatusHealthyBg = Color(0xFF064E3B) // Emerald 900
val StatusClosed = Color(0xFFF59E0B) // Amber 500
val StatusClosedBg = Color(0xFF78350F) // Amber 900
val StatusUnreachable = Color(0xFFEF4444) // Rose 500
val StatusUnreachableBg = Color(0xFF7F1D1D) // Rose 900
val StatusNeutral = Color(0xFF6B7280) // Gray 500
val StatusNeutralBg = Color(0xFF1F2937) // Gray 800

// Container States
val ContainerRunning = Color(0xFF10B981)
val ContainerPaused = Color(0xFFF59E0B)
val ContainerExited = Color(0xFFEF4444)
val ContainerRestarting = Color(0xFF8B5CF6)
val ContainerNotFound = Color(0xFF6B7280)

// Brand & Accents
val PrimaryBlue = Color(0xFF3B82F6)
val PrimaryBlueHover = Color(0xFF60A5FA)
val PrimaryBlueBg = Color(0xFF1E3A8A)
val AccentCyan = Color(0xFF06B6D4)
val AccentPurple = Color(0xFF8B5CF6)

// Text Colors
val TextPrimary = Color(0xFFF3F4F6)
val TextSecondary = Color(0xFF9CA3AF)
val TextMuted = Color(0xFF6B7280)
val TextOnAccent = Color(0xFFFFFFFF)

// Terminal / Console
val ConsoleBackground = Color(0xFF0A0C10)
val ConsoleGutter = Color(0xFF16181F)
val ConsoleGutterText = Color(0xFF4B5563)
val ConsoleText = Color(0xFFE5E7EB)
val ConsoleHighlightBg = Color(0xFF854D0E) // Yellow-Brown highlight
val ConsoleHighlightText = Color(0xFFFEF08A)
val ConsoleSelection = Color(0xFF1E3A8A)
